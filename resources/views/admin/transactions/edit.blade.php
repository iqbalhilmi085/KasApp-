        ...
        <x-card title="Edit Transaksi">
            ...
            >
                ...
                <div>
                    ...
                    <input type="date" id="transaction_date" name="transaction_date" value="{{ old('transaction_date', $transaction->transaction_date->format('Y-m-d')) }}" max="{{ date('Y-m-d') }}" class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1e3a5f]/20 focus:border-[#1e3a5f] outline-none transition-colors @error('transaction_date') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Keterangan transaksi..." class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1e3a5f]/20 focus:border-[#1e3a5f] outline-none transition-colors @error('description') border-red-500 @enderror">{{ old('description', $transaction->description) }}</textarea>
                ...
                <div>
                    ...
                    @if ($transaction->attachment)
                        <div class="flex items-center gap-3 mb-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                            @if (str_starts_with($transaction->attachment, 'attachments/') && in_array(pathinfo($transaction->attachment, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png']))
                                <img src="{{ Storage::url($transaction->attachment) }}" class="w-16 h-16 object-cover rounded-lg border border-gray-200">
                            @else
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-900 truncate">{{ basename($transaction->attachment) }}</p>
                                <a href="{{ Storage::url($transaction->attachment) }}" target="_blank" class="text-xs text-blue-600 hover:underline">Lihat file</a>
                            </div>
                        </div>
                    @endif
