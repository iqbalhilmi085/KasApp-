        ...
        <x-card title="Edit Kategori">
            <form method="POST" action="{{ route('categories.update', $category->id) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $category->name) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1e3a5f] @error('name') border-red-500 @enderror" required max="100">
